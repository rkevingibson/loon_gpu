...
typedef void (*WGPULoonProcLogCallback)(WGPULoonLogLevel lvl, StringView message, void* userdata);
...
class Device {
   public:
    ...
    GpuTextureDescriptor textureViewDescriptor(Handle<Texture> texture, GpuViewDesc desc);
    GpuTextureDescriptor RWTextureViewDescriptor(Handle<Texture> texture, GpuViewDesc desc);
    void                 free(Handle<Texture>);

    // Pipelines
    Handle<Pipeline> createComputePipeline(ByteSpan computeIR);
    Handle<Pipeline> createGraphicsPipeline(ByteSpan      vertexIR,
                                            ByteSpan      pixelIR,
                                            GpuRasterDesc desc);
...
void gpuWaitBefore(Handle<CommandBuffer> cb, STAGE after, GpuPtr ptrGpu,
    ...
    void beginRenderPass(GpuRenderPassDesc desc);
    void endRenderPass();
