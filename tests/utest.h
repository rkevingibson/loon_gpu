...
static UTEST_INLINE void* utest_realloc(void* const pointer, size_t new_size) {
    ...
    if (UTEST_NULL == new_pointer) { free(pointer); }
...
#endif
#define UTEST_PRINTF(...)                                                                          \
    if (utest_state.output) { fprintf(utest_state.output, __VA_ARGS__); }                          \
    printf(__VA_ARGS__)
#ifdef __clang__
#    pragma clang diagnostic pop
#endif
...
template <>
struct utest_type_deducer<char, false> {
...
struct utest_type_deducer<signed char, false> {
    static void _(const signed char c) { UTEST_PRINTF("%d", static_cast<int>(c)); }
};

template <>
struct utest_type_deducer<unsigned char, false> {
    static void _(const unsigned char c) { UTEST_PRINTF("%u", static_cast<unsigned int>(c)); }
};
...
struct utest_type_deducer<unsigned short, false> {
    static void _(const unsigned short s) { UTEST_PRINTF("%u", static_cast<unsigned>(s)); }
};
...
struct utest_type_deducer<const T*, false> {
    static void _(const T* t) { UTEST_PRINTF("%p", static_cast<const void*>(t)); }
};
...
struct utest_type_deducer<T, true> {
    static void _(const T t) { UTEST_PRINTF("%llu", static_cast<unsigned long long>(t)); }
};

// default printer for all other objects (specialize for custom printing)
template <typename T>
struct utest_type_deducer<T, false> {
    static void _(const T& t) { UTEST_PRINTF("(object %p)", static_cast<const void*>(&t)); }
};

template <>
struct utest_type_deducer<std::nullptr_t, false> {
    static void _(std::nullptr_t t) { UTEST_PRINTF("%p", static_cast<void*>(t)); }
};
...
#    define UTEST_STRNCMP(x, y, size)                                                              \
        _Pragma("clang diagnostic push")                                                           \
            _Pragma("clang diagnostic ignored \"-Wdisabled-macro-expansion\"") strncmp(x, y, size) \
                _Pragma("clang diagnostic pop")
#else
...
#    define UTEST_COND(x, y, cond, msg, is_assert)                                                 \
        UTEST_SURPRESS_WARNING_BEGIN do {                                                          \
            if (!((x)cond(y))) {                                                                   \
                UTEST_PRINTF("%s:%i: Failure (Expected " #cond " Actual)", __FILE__, __LINE__);    \
                if (strlen(msg) > 0) { UTEST_PRINTF(" Message : %s", msg); }                       \
                UTEST_PRINTF("\n");                                                                \
                *utest_result = UTEST_TEST_FAILURE;                                                \
                if (is_assert) { return; }                                                         \
            }                                                                                      \
...
#    define UTEST_EXCEPTION(x, exception_type, msg, is_assert)                                     \
        UTEST_SURPRESS_WARNING_BEGIN do {                                                          \
            ...
            try {                                                                                  \
                x;                                                                                 \
            } catch (const exception_type&) { exception_caught = 1; } catch (...) {                \
                exception_caught = 2;                                                              \
            }                                                                                      \
            if (1 != exception_caught) {                                                           \
                ...
                UTEST_PRINTF("    Actual : %s\n",                                                  \
                             (2 == exception_caught) ? "Unexpected exception" : "No exception");   \
                if (strlen(msg) > 0) { UTEST_PRINTF("   Message : %s\n", msg); }                   \
                *utest_result = UTEST_TEST_FAILURE;                                                \
                if (is_assert) { return; }                                                         \
            }                                                                                      \
...
#    define EXPECT_EXCEPTION(x, exception_type)          UTEST_EXCEPTION(x, exception_type, "", 0)
#    define EXPECT_EXCEPTION_MSG(x, exception_type, msg) UTEST_EXCEPTION(x, exception_type, msg, 0)
#    define ASSERT_EXCEPTION(x, exception_type)          UTEST_EXCEPTION(x, exception_type, "", 1)
#    define ASSERT_EXCEPTION_MSG(x, exception_type, msg) UTEST_EXCEPTION(x, exception_type, msg, 1)

#    define UTEST_EXCEPTION_WITH_MESSAGE(x, exception_type, exception_message, msg, is_assert)     \
        UTEST_SURPRESS_WARNING_BEGIN do {                                                          \
            int   exception_caught = 0;                                                            \
            ...
            try {                                                                                  \
                ...
                exception_caught       = 1;                                                        \
                if (0 != UTEST_STRNCMP(what, exception_message, strlen(exception_message))) {      \
...
#    define EXPECT_EXCEPTION_WITH_MESSAGE_MSG(x, exception_type, exception_message, msg)           \
...
#    define ASSERT_EXCEPTION_WITH_MESSAGE_MSG(x, exception_type, exception_message, msg)           \
...
#define UTEST_F_SETUP(FIXTURE)                                                                     \
    static void utest_f_setup_##FIXTURE(int* utest_result, struct FIXTURE* utest_fixture)
...
#define UTEST_F(FIXTURE, NAME)                                                                     \
    ...
    static void utest_f_##FIXTURE##_##NAME(int* utest_result, size_t utest_index) {                \
        ...
        if (UTEST_TEST_PASSED != *utest_result) { return; }                                        \
...
#define UTEST_I_SETUP(FIXTURE)                                                                     \
    static void utest_i_setup_##FIXTURE(int*            utest_result,                              \
                                        struct FIXTURE* utest_fixture,                             \
                                        size_t          utest_index)

#define UTEST_I_TEARDOWN(FIXTURE)                                                                  \
    static void utest_i_teardown_##FIXTURE(int*            utest_result,                           \
                                           struct FIXTURE* utest_fixture,                          \
                                           size_t          utest_index)
    ...
    if (filter) {
        ...
        while (('\0' != *filter_cur) && ('\0' != *testcase_cur)) {
            if ('*' == *filter_cur) {
                ...
                if (('\0' == *filter_cur) && ('\0' == *testcase_cur)) { return 0; }

                /* if the testcase has been exhausted, we don't have a match! */
                if ('\0' == *testcase_cur) { return 1; }
            } else {
...
int                     utest_main(int argc, const char* const argv[]) {
    ...
    for (index = 0; index < utest_state.tests_length; index++) {
        if (utest_should_filter_test(filter, utest_state.tests[index].name)) { continue; }
    ...
    for (index = 0; index < utest_state.tests_length; index++) {
        ...
        if (utest_state.output) { fprintf(utest_state.output, "</testcase>\n"); }
    ...
    if (utest_state.output) { fprintf(utest_state.output, "</testsuite>\n</testsuites>\n"); }
...
cleanup:
    ...
    if (utest_state.output) { fclose(utest_state.output); }
