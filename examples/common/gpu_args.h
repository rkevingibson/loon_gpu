...
class RingBuffer {
   public:
    ...
    template <class T>
    gpu::GpuSpan append(uint64_t frame_idx, const T&);

    template <class T>
    gpu::GpuSpan append(uint64_t frame_idx, const std::vector<T>&);

    gpu::GpuSpan append_raw(uint64_t    frame_idx,
                            const void* ptr,
                            size_t      size,
...
template <class T>
gpu::GpuSpan RingBuffer::append(uint64_t frame_idx, const T& t) {
    return append_raw(frame_idx, &t, sizeof(T), alignof(T));
}

template <class T>
gpu::GpuSpan RingBuffer::append(uint64_t frame_idx, const std::vector<T>& vec) {
